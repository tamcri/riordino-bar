...
function formatNum(n: number) {
  return new Intl.NumberFormat("it-IT", { minimumFractionDigits: 0, maximumFractionDigits: 3 }).format(n || 0);
}
...
export default function ProgressiviReportTable({ data, downloadHref }: { data: ProgressiviReportData; downloadHref?: string }) {
  return (
    <div className="space-y-4">
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
          <div>
            ...
            <div className="mt-1 text-sm text-gray-600">
              Label: <span className="font-medium text-gray-800">{data.current_header.label || "—"}</span>
            </div>
            <div className="mt-1 text-xs text-gray-500">
              Inventario corrente: {formatDateIT(data.current_header.inventory_date)} · Inventario precedente: {formatDateIT(data.previous_header?.inventory_date)}
            </div>
          </div>

          {downloadHref ? (
            <a className="inline-flex items-center justify-center rounded-xl bg-slate-900 px-4 py-2 text-sm text-white hover:bg-slate-800" href={downloadHref}>
              Scarica Excel
            </a>
          ) : null}
        </div>
      ...
      <div className="rounded-2xl border bg-white p-4 text-xs text-amber-800 bg-amber-50 border-amber-200">
        ...
        <table className="min-w-[1500px] w-full text-sm">
          <thead>
            <tr className="bg-gray-100">
              ...
              <th colSpan={5} className="border-b border-r p-3 text-center font-semibold">{formatDateIT(data.previous_header?.inventory_date)}</th>
              <th colSpan={5} className="border-b border-r p-3 text-center font-semibold">{formatDateIT(data.current_header.inventory_date)}</th>
              <th colSpan={2} className="border-b p-3 text-center font-semibold">RISCONTRO</th>
            </tr>
          ...
          <tfoot>
            <tr className="bg-slate-100 font-semibold">
              <td className="border-t border-r p-3" colSpan={3}>Totali</td>
