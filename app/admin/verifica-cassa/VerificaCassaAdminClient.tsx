...
export default function VerificaCassaAdminClient() {
  ...
  async function loadData() {
    try {
      const res = await fetch(
        `/api/admin/cash-registers/list?t=${Date.now()}`,
        {
          cache: 'no-store',
        },
      );
      ...
      console.error(
        'Errore caricamento verifica cassa:',
        error,
      );
  ...
  async function handleSave(
    ...
    try {
      ...
      const nextDate =
        savedItem?.next_verification_date ??
        calculateNextVerificationDate(
          value || null,
        );

      const lastDate =
        savedItem?.last_verification_date ??
        (value || null);
  ...
  async function handleQrUpload(
    ...
    try {
      ...
      const res =
        await fetch(
          '/api/admin/cash-registers/upload-qr',
          {
            method:
              'POST',
            body: formData,
  ...
  async function handleDisableCash(
    ...
    if (
      ...
      alert(
        'Cassa 1 non può essere disattivata',
      );
    ...
    const confirmed =
      window.confirm(
        `Vuoi disattivare ${item.label} di ${item.pv_code} - ${item.pv_name}?`,
      );
    ...
    try {
      ...
      const res =
        await fetch(
          '/api/admin/cash-registers/toggle-second',
          {
            method:
              'POST',
            headers: {
      ...
      if (!res.ok) {
        alert(
          data?.error ||
            `Errore disattivazione ${item.label}`,
        );
      ...
      setCashActionMessage(
        data?.message ||
          `${item.label} disattivata`,
      );

      await loadData();
    } catch (error) {
      console.error(
        'Errore disattivazione cassa:',
        error,
      );
  ...
  async function handleDownloadReport() {
    try {
      ...
      const res =
        await fetch(
          '/api/admin/cash-registers/report',
          {
            method:
              'GET',
            cache:
        ...
        <div className="mb-6 rounded-2xl border border-gray-200 bg-gray-50 p-4 shadow-sm">
          ...
          <div className="flex flex-col gap-3 md:flex-row md:items-center">
            <select
              ...
              className="min-w-[260px] rounded-xl border border-gray-300 bg-white px-3 py-2 text-sm text-gray-700"
            >
              <option value="">
            ...
            <button
              ...
              className="w-fit rounded-xl border border-gray-900 bg-gray-900 px-4 py-2 text-sm font-medium text-white hover:bg-gray-800 disabled:cursor-not-allowed disabled:opacity-50"
            >
              {addingCash
                ? 'Aggiunta in corso...'
                : '+ Aggiungi Cassa'}
