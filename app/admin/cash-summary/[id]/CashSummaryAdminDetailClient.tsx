...
type MetricChecksMap = Partial<Record<FieldCommentKey, CheckState>>;
...
export default function CashSummaryAdminDetailClient({ id }: { id: string }) {
  ...
  const [metricChecks, setMetricChecks] = useState<MetricChecksMap>({});
  const [verificationSavingKey, setVerificationSavingKey] =
    useState<FieldCommentKey | null>(null);
  ...
  const [selectedSupplier, setSelectedSupplier] =
    useState<SupplierSearchRow | null>(null);
  ...
  async function loadDetail() {
    ...
    try {
      const res = await fetch(
        `/api/cash-summary/admin-get?id=${encodeURIComponent(id)}`,
        {
          cache: "no-store",
        }
      );
      ...
      setFieldComments(
        json?.field_comments &&
          typeof json.field_comments === "object"
          ? json.field_comments
          : {}
      );
      ...
      setMetricChecks(
        json?.metric_checks &&
          typeof json.metric_checks === "object"
          ? json.metric_checks
          : {}
      );
    } catch {
  ...
  async function toggleFieldVerification(fieldKey: FieldCommentKey) {
    if (!summary) return;

    const currentStatus = metricChecks[fieldKey] ?? null;
    const nextStatus = currentStatus === "ok" ? null : "ok";

    setVerificationSavingKey(fieldKey);
    setMsg(null);

    const previousChecks = metricChecks;
    ...
    try {
      ...
      if (!res.ok || !json?.ok) {
        setMetricChecks(previousChecks);
        setMsg(json?.error || "Errore salvataggio verifica campo");
        return;
      }
    } catch {
      setMetricChecks(previousChecks);
      setMsg("Errore di rete durante il salvataggio della verifica");
    } finally {
      setVerificationSavingKey(null);
    }
  }
  ...
  async function searchSuppliers(q: string) {
    ...
    try {
      const res = await fetch(
        `/api/suppliers/search?q=${encodeURIComponent(q)}`,
        {
          cache: "no-store",
        }
      );
  ...
  const daVersare = useMemo(() => {
    ...
    return roundMoney(
      summary.tot_versato === null ||
        n(summary.tot_versato) === 0
        ? versamento
  ...
  const fondoDeltaPercent = useMemo(() => {
    ...
    return computeFondoDeltaPercent(
      summary.fondo_cassa_iniziale,
      summary.fondo_cassa
    );
  }, [summary]);
      ...
      <section className="rounded-2xl border bg-white p-4">
        ...
        <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-3">
          <EditableNumberField
            ...
            onCommentClear={() =>
              clearFieldComment("incasso_totale")
            }
            verified={
              metricChecks.incasso_totale === "ok"
            }
            verificationSaving={
              verificationSavingKey ===
              "incasso_totale"
            }
            onToggleVerified={() =>
          ...
          <EditableNumberField
            ...
            onCommentClear={() =>
              clearFieldComment("gv_pagati")
            }
            verified={
              metricChecks.gv_pagati === "ok"
            }
            verificationSaving={
              verificationSavingKey ===
              "gv_pagati"
          ...
          <EditableNumberField
            ...
            onCommentClear={() =>
              clearFieldComment("lis_plus")
            }
            verified={
              metricChecks.lis_plus === "ok"
            }
            verificationSaving={
              verificationSavingKey ===
              "lis_plus"
          ...
          <EditableNumberField
            ...
            onCommentClear={() =>
              clearFieldComment("mooney")
            }
            verified={
              metricChecks.mooney === "ok"
            }
            verificationSaving={
              verificationSavingKey ===
              "mooney"
      ...
      <section className="rounded-2xl border bg-white p-4">
        ...
        <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-3">
          ...
          <EditableNumberField
            ...
            comment={
              fieldComments.vendita_tabacchi ??
              ""
            }
            onCommentChange={(value) =>
              updateFieldComment(
          ...
          <Field
            label="Versamento"
            value={formatEuro(versamento)}
          />

          <Field
            label="Da Versare"
            value={formatEuro(daVersare)}
          />
      ...
      <section className="rounded-2xl border bg-white p-4">
        ...
        <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-3">
          <EditableNumberField
            ...
            comment={
              fieldComments.fondo_cassa_iniziale ??
              ""
            }
            onCommentChange={(value) =>
              updateFieldComment(
            ...
            verified={
              metricChecks.fondo_cassa_iniziale ===
              "ok"
            }
            verificationSaving={
              verificationSavingKey ===
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        setTimeout(
                          () => {
                            const input =
                              supplierAmountInputRef.current;
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-3">
        ...
        {onToggleVerified && (
          <button
            ...
            aria-label={
              verified
                ? `Rimuovi verifica ${label}`
                : `Verifica ${label}`
            }
            className={`inline-flex h-7 min-w-7 items-center justify-center rounded-full border px-2 text-xs font-semibold transition disabled:opacity-50 ${
              verified
                ? "border-green-600 bg-green-600 text-white hover:bg-green-700"
                : "border-gray-300 bg-white text-gray-500 hover:border-green-500 hover:bg-green-50 hover:text-green-700"
            }`}
          >
            {verificationSaving
              ? "..."
      ...
      <input
        type="number"
        step="0.01"
        className={`mt-1 w-full rounded-lg border p-2 transition ${
          verified
