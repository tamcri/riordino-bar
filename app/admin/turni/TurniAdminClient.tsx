...
type ViewMode = "weekly" | "monthly" | "analysis";
...
export default function TurniAdminClient() {
  ...
  const [analysisAlerts, setAnalysisAlerts] = useState<AnalysisAlert[]>([]);
  const [analysisTotals, setAnalysisTotals] = useState<AnalysisResponse["totals"] | null>(null);
  ...
  async function loadAnalysisRows() {
    ...
    try {
      ...
      setAnalysisAlerts(Array.isArray(res.data?.alerts) ? res.data.alerts : []);
  ...
  function resetAnalysisFilters() {
    setAnalysisPvId("");
    setAnalysisMonth(currentMonthValue());
    setAnalysisRows([]);
    setAnalysisAlerts([]);
    setAnalysisTotals(null);
  }
  ...
  return (
    <main className="min-h-screen bg-gray-100">
      <div className="mx-auto max-w-7xl px-6 py-6 space-y-6">
        ...
        {viewMode === "analysis" && (
          <section className="rounded-2xl border bg-white p-4">
            <div className="grid grid-cols-1 gap-4 md:grid-cols-[1fr_1.4fr_auto_auto] md:items-end">
              <div>
                <label className="block text-sm font-medium mb-2">Mese</label>
                <input
                  type="month"
        ...
        {viewMode === "weekly" ? (
          ...
          <section className="rounded-2xl border bg-white overflow-hidden">
            <div className="border-b p-4">
              <h2 className="text-lg font-semibold">Analisi equilibrio turni</h2>
              <p className="text-sm text-gray-600 mt-1">
                Confronta ore, mattine, pomeriggi, notti e spezzati tra i dipendenti dello stesso punto vendita.
              </p>
              {analysisTotals && (
                <div className="mt-3 grid grid-cols-1 gap-3 md:grid-cols-4">
                  <div className="rounded-xl border bg-gray-50 p-3">
                    <div className="text-xs text-gray-500">Ore totali</div>
                    <div className="text-lg font-semibold">{analysisTotals.total_hours_label ?? formatHours(Number(analysisTotals.total_hours ?? 0))} h</div>
                  </div>
                  <div className="rounded-xl border bg-gray-50 p-3">
                    <div className="text-xs text-gray-500">Media ore</div>
                    <div className="text-lg font-semibold">{analysisTotals.avg_hours_label ?? formatHours(Number(analysisTotals.avg_hours ?? 0))} h</div>
                  </div>
                  <div className="rounded-xl border bg-gray-50 p-3">
                    <div className="text-xs text-gray-500">Notti totali / media</div>
                    <div className="text-lg font-semibold">{analysisTotals.total_nights ?? 0} / {formatHours(Number(analysisTotals.avg_nights ?? 0))}</div>
                  </div>
                  <div className="rounded-xl border bg-gray-50 p-3">
                    <div className="text-xs text-gray-500">Spezzati totali / media</div>
                    <div className="text-lg font-semibold">{analysisTotals.total_splits ?? 0} / {formatHours(Number(analysisTotals.avg_splits ?? 0))}</div>
                  </div>
                </div>
              )}
            </div>
            ...
            {!analysisPvId ? (
              <div className="p-6 text-sm text-gray-600">Seleziona un punto vendita e clicca Analizza.</div>
            ) : analysisLoading ? (
              ...
              <>
                ...
                <div className="overflow-x-auto">
                  <table className="min-w-full text-sm">
                    <thead className="bg-gray-50 text-left">
                      <tr>
                        <th className="border-b px-3 py-3 font-semibold min-w-52">Dipendente</th>
                      ...
                      {analysisRows.map((row) => (
                        <tr key={row.employee_id} className="hover:bg-gray-50">
                          <td className="border-b px-3 py-3">
                            ...
                            {!row.employee_active && <div className="text-xs text-gray-500">Non attivo</div>}
                          </td>
                          <td className="border-b px-3 py-3 text-right font-semibold">{row.total_hours_label} h</td>
                          <td className="border-b px-3 py-3 text-right">{row.mornings}</td>
                          <td className="border-b px-3 py-3 text-right">{row.afternoons}</td>
                          <td className="border-b px-3 py-3 text-right font-semibold">{row.nights}</td>
