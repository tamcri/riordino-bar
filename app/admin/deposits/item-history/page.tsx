      ...
      <div className="mx-auto max-w-6xl px-6 py-6 space-y-6">
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">Punti di inventario nel tempo. Se non inventari un giorno, non esiste il punto.</p>
          </div>
        ...
        <section className="rounded-2xl border bg-white p-4 space-y-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 items-end">
            <div>
              ...
              <input type="date" className="w-full rounded-xl border p-3" value={from} onChange={(e) => setFrom(e.target.value)} />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">A</label>
              <input type="date" className="w-full rounded-xl border p-3" value={to} onChange={(e) => setTo(e.target.value)} />
            </div>
            <button
              type="button"
          ...
          {!summary ? (
            ...
            <div className="mt-3 grid grid-cols-1 md:grid-cols-4 gap-3">
              <div className="rounded-xl border p-3 bg-white">
                ...
                <div className="text-lg font-semibold">{summary.qty_from} {summary.unit}</div>
                <div className="text-xs text-gray-500">Data: {summary.from ?? "—"}</div>
              </div>
              <div className="rounded-xl border p-3 bg-white">
                <div className="text-xs text-gray-600">Ultimo (≤ A)</div>
                <div className="text-lg font-semibold">{summary.qty_to} {summary.unit}</div>
                <div className="text-xs text-gray-500">Data: {summary.to ?? "—"}</div>
              </div>
              <div className="rounded-xl border p-3 bg-white">
                <div className="text-xs text-gray-600">Differenza</div>
                <div className="text-lg font-semibold">{summary.diff >= 0 ? "+" : ""}{summary.diff} {summary.unit}</div>
          ...
          {!item ? (
            ...
            <div className="mt-3 overflow-auto">
              <svg width={chart?.w ?? 700} height={chart?.h ?? 220} className="rounded-xl border bg-white">
                <line x1={24} y1={(chart?.h ?? 220) - 18} x2={(chart?.w ?? 700) - 24} y2={(chart?.h ?? 220) - 18} stroke="#e5e7eb" />
              ...
              <div className="text-xs text-gray-600 mt-2">Unità grafico: <b>{chart?.unit}</b></div>
            </div>
          )}
        </section>
