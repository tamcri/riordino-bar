...
export default function InventoryPvClient() {
  ...
  const [pvInfo, setPvInfo] = useState<Pv | null>(null);
...
if (isRapidUrl && rapidId) {
  ...
  // ✅ in Rapido non bloccare sul caricamento items: blocca su almeno 1 riga “valida”
  // Se hai già una computed rowsValidCount usala, altrimenti usiamo scannedIds.length
  const rapidOk =
    inventoryMode !== "rapid"
      ? true
      : (scannedIds?.length || 0) > 0;

  return !!operatore.trim() && !!pvId && catOk && rapidOk;
}, [operatore, pvId, categoryId, inventoryMode, scannedIds]);
  ...
  async function loadMePv() {
  const res = await fetch("/api/me", { cache: "no-store" });
  const json: MeResponse | null = await res.json().catch(() => null);

  if (!res.ok || !json?.ok) throw new Error(json?.error || "Errore sessione");
  if (json.role !== "punto_vendita") throw new Error("Accesso negato");
  if (!json.pv_id) throw new Error("PV non assegnato a questo utente");

  setPvId(json.pv_id);

  const code = String(json.pv_code || "").trim();
  const name = String(json.pv_name || "").trim();

  setPvInfo({
    id: json.pv_id,
    code: code || json.pv_id,   // fallback ultra safe
    name: name || "",
    is_active: true,
  });
}
...
() {
  ...
  // ✅ PV: sempre Rapido → carico tutti gli articoli (categorie ignorate)
  ...
  useEffect(() => {
    (async () => {
      ...
      setError(null);
      try {
        await Promise.all([loadMePv(), loadCategories()]);
      } catch (e: any) {
      ...
      <div className="rounded-2xl border bg-white p-4 grid grid-cols-1 md:grid-cols-4 gap-3">
        <div>
          ...
          <input
            ...
            value={pvInfo ? `${pvInfo.code} — ${pvInfo.name}` : pvId}
            disabled
          />
        </div>
